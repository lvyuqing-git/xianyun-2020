      ...
      <el-col :span="17"
        ...
        <client-only>
          <vue-editor v-model="content"
                      class="create-editor"></vue-editor>
        </client-only>
        <!-- 选择城市 -->
        <div class="create-city">
          <span>选择城市</span>
          <el-autocomplete v-model="cityKey"
                           :fetch-suggestions="querySearchAsync"
                           placeholder="请选择游玩城市"
                           @select="handleSelect"
                           @blur="handleBlur"></el-autocomplete>
        </div>
        <!-- 操作区 -->
        <div class="create-operate">
          ...
          <span>或者</span>
          <span @click="save" class="cun">保存到草稿</span>
        </div>
      </el-col>
      <!-- 右侧- 草稿箱 -->
      <el-col :span="6"
              class="right-draft">
        <CreateDraft @caogao='fugao'></CreateDraft>
      </el-col>
    </el-row>
  </div>
</template>
...
<script>
import CreateDraft from '@/components/post/createDraft'
...
export default {
  ...
  methods: {
      ...
      fugao(item){
          ...
          this.cityKey = item.cityKey
      },
      //保存草稿
      save(){
          this.$store.commit('post/setArticleInfo',{
              title : this.title,
